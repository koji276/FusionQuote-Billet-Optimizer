...
# --- メイン：データ入力（フォーム化） ---
# st.formを使うことで、テキストエリアで「Ctrl+Enter」を押した際に自動送信されます
...
{{
  ...
  "finish_weight": 8.5
    ...
    # フォーム専用の送信ボタン（これがCtrl+Enterで反応します）
    submit_btn = st.form_submit_button("Generate Strategic Quote")
...
if submit_btn:
    if json_str:
        try:
            ...
            # { が抜けているなどのエラー時はここに入ります
            st.error(f"JSON解析エラー: 入力内容を確認してください。({e})")
    else:
